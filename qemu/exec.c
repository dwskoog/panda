                                      ...
                                      bool readonly)
{
    ...
    if (rr_in_record() && rr_record_in_progress) {
        target_phys_addr_t start_addr = now.offset_within_address_space;
        ram_addr_t size = now.size;
        ram_addr_t phys_offset now.mr->ram_addr;
        ...
        if (is_write) {
                ...
                if (l >= 4 && ((addr1 & 3) == 0)) {
                    ...
                    if (io_index == (IO_MEM_NOTDIRTY >> IO_MEM_SHIFT)) {
                      ...
                      RR_DO_RECORD_OR_REPLAY(
                        /*action=*/io_mem_write(section->mr, addr1, val, 4),
                        /*record=*/RR_NO_ACTION,
                        /*replay=*/RR_NO_ACTION,
                    ...
                    if (io_index == (IO_MEM_NOTDIRTY >> IO_MEM_SHIFT)) {
                      ...
                      RR_DO_RECORD_OR_REPLAY(
                        /*action=*/io_mem_write(section->mr, addr1, val, 2),
                        /*record=*/RR_NO_ACTION,
                        /*replay=*/RR_NO_ACTION,
                    ...
                    if (io_index == (IO_MEM_NOTDIRTY >> IO_MEM_SHIFT)) {
                      ...
                      RR_DO_RECORD_OR_REPLAY(
                        /*action=*/io_mem_write(section->mr, addr1, val, 1),
                        /*record=*/RR_NO_ACTION,
                        /*replay=*/RR_NO_ACTION,
            ...
            if (!(memory_region_is_ram(section->mr) ||
                ...
                if (l >= 4 && ((addr1 & 3) == 0)) {
                    /* 32 bit read access */
                    RR_DO_RECORD_OR_REPLAY(
                        /*action=*/val = io_mem_read(section->mr, addr1, 4),
                    ...
                    /* 16 bit read access */
                    RR_DO_RECORD_OR_REPLAY(
                        /*action=*/val = io_mem_read(section->mr, addr1, 2),
                    ...
                    /* 8 bit read access */
                    RR_DO_RECORD_OR_REPLAY(
                        /*action=*/val = io_mem_read(section->mr, addr1, 1),
                ...
                //TODO: triple check this after rebuild from v1.1.0
                ram_addr_t addr1 = memory_region_get_ram_addr(section->mr)
                  + memory_region_section_addr(section, addr);
                if (rr_mode == RR_REPLAY) {
                    // run all callbacks registered for cpu_physical_memory_rw ram case
                    panda_cb_list *plist;
                                         ...
                                         enum device_endian endian)
{
    ...
    if (!(memory_region_is_ram(section->mr) ||
        ...
        RR_DO_RECORD_OR_REPLAY(
            /*action=*/val = val = io_mem_read(section->mr, addr, 4),
            /*record=*/rr_input_4(&val),
            /*replay=*/rr_input_4(&val),
...
#endif
        //TODO: recheck this against v1.0.1
        if (!memory_region_get_dirty(section->mr, addr, 8, DIRTY_MEMORY_MIGRATION)) {
            ACTION;
        }
        else {
            RR_DO_RECORD_OR_REPLAY(
...
void stl_phys_notdirty(target_phys_addr_t addr, uint32_t val)
{
    ...
    if (!memory_region_is_ram(section->mr) || section->readonly) {
        ...
        if (!memory_region_get_dirty(section->mr, addr, 8, DIRTY_MEMORY_MIGRATION)) {
          ...
          RR_DO_RECORD_OR_REPLAY(
            /*action=*/io_mem_write(section->mr, addr, val, 4),
            /*record=*/RR_NO_ACTION,
            /*replay=*/RR_NO_ACTION,
...
/* virtual memory access for debug (includes writing to ROM) */
int cpu_memory_rw(CPUArchState *env, target_ulong addr,
                  uint8_t *buf, int len, int is_write)
{
    int l;
    target_phys_addr_t phys_addr;
...
target_phys_addr_t panda_virt_to_phys(CPUArchState *env, target_ulong addr){
    target_ulong page;
    target_phys_addr_t phys_addr;
    page = addr & TARGET_PAGE_MASK;
    phys_addr = cpu_get_phys_page_debug(env, page);
...
int panda_virtual_memory_rw(CPUArchState *env, target_ulong addr,
                        uint8_t *buf, int len, int is_write)
{
    int l;
    int ret;
